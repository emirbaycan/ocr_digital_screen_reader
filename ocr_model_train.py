...
class OCRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx % 10 == 0:
            ...
            img_uint8 = (img.cpu().numpy().squeeze() * 255).astype(np.uint8)  # Convert to uint8
            cv2.imwrite(debug_img_path, img_uint8)

        return img, torch.tensor(label_indices, dtype=torch.long)

def decode_ctc(predictions):
    """
    Decodes the CTC output by removing blank tokens and collapsing repeated characters.
    """
    decoded = []
    for pred in predictions:
        if pred.dim() == 0:  # Ensure pred is iterable
        ...
        for idx in pred:
            ...
            if char != "<BLANK>" and char != prev_char:  # Remove <BLANK> and repeated characters
                text.append(char)
            prev_char = char
        
        decoded.append("".join(text))  # Convert list to string
    return decoded
...
if __name__ == "__main__":
    ...
    ACCURACY_THRESHOLD = 0.99  # Stop training if accuracy reaches this value
    ...
    num_workers = min(8, os.cpu_count())  # Use max 4 workers or available CPU cores
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        ...
        try:
            ...
            print("⚠️ Possible model architecture mismatch. Re-training from scratch.")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        model.train()
        train_loss, correct_sequences, total_sequences = 0, 0, 0
        ...
        for images, labels in train_loader:
            ...
            # 🔹 Character-level Accuracy Calculation (Updated)
            predictions = torch.argmax(outputs, dim=2)  # Get predicted indices

            # Decode predictions and true labels
            decoded_predictions = decode_ctc(predictions)
            decoded_truths = ["".join([idx_to_char[idx.item()] for idx in label if idx.item() in idx_to_char]) for label in labels]

            # Compare decoded words
            correct_sequences += sum(1 for pred, true_label in zip(decoded_predictions, decoded_truths) if pred == true_label)
            total_sequences += len(labels)

        # Compute overall sequence accuracy
